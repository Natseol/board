...
public class CommentDAOMysql {
	...
	public List<Comment> getComments(int boardId) {
		return jdbcTemplate.query("select * from comments where board_id=? order by id desc limit 0,5", mapper, boardId);
	}	 
	...
	public List<Comment> getParent(int boardId, int start) {
		return jdbcTemplate.query("select a.*, b.user_id as userStrId"
				...
				+ " order by a.id desc limit ?,5", mapper, boardId, start);
	}
